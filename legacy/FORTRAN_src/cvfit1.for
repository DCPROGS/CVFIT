...
c  norm1=true do means plotted also in CVDISP -no option yet to use SD
...
c	dimension ncal(20),icurvc(20),iline(20)		!for calc curve
	...
	logical noguess(100),doframe,screen,errflag
...
c next bit now done in CVIN2
c	nsfit=0
c	do j=1,nset
c	   if(jmiss(j).eq.0) then
c	      nsfit=nsfit+1
c	      juse(nsfit)=j
c	   endif
c	enddo
...
c For ifitmode=4,5: kmax redefined in GETEQN as is kmax1=orig no of param
	...
	call CVSIMP(kfit,jfix,nfix,titlep,fitted,dcurve,
     ...
     & xgt0,xmin,xmax,ymin,ymax,autosep,iparsav,jfixsav,sminsav,
     & neweq,guesdone,noguess,mset,ibk)
...
c=to be fixed
c=		do 44 i=2,ip1+1
c=44		thetsav(i)=thetsav(i)/(ratio**(i-1))
	   endif
c
