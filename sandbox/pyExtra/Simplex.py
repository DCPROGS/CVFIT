...
class Simplex:
    def __init__(self, testfunc, func, guess, data, opt, increments, kR = -1, kE = 2, kC = 0.5):
    ...
    def minimize(self, epsilon = 0.0001, maxiters = 250, monitor = 1):
        ...
        for iter in range(0, maxiters):
            ...
            if T <= epsilon:   # We converged!  Break out of loop!
                ...
                self.currenterror = self.testfunc(self.guess, self.data, self.opt, self.func)
...
def objective_function(args, data, opt, func):
...
def main():
    ...
    data, theta, opt = pf.prepare()




    s = Simplex(pf.ssdcv, pf.eqnHill, theta, data, opt, [0, 1, 5, 5])
    values, err, iter = s.minimize()
    print 'args = ', values
    print 'error = ', err
    print 'iterations = ', iter

    pf.plot(values, data, opt)
